...
| 5 | Sharing & Deploy | 4/4 | Complete — Human Verified | 2026-05-19 |
| 6 | Google Maps Migration & World Map | Migrate viewer map to @vis.gl/react-google-maps; replace landing page WorldMapSection with D3 arc animation | — | 3 |
...
- [x] 05-01-PLAN.md — Supabase RLS policy: allow anon reads on trips WHERE is_public = true

**Wave 1** *(parallel — disjoint files)*
- [x] 05-02-PLAN.md — Public viewer route group (public)/t/[id], TrailfilmWatermark, ArchiveCTA, CinematicViewer showNav prop, OG meta tags, middleware comment
- [x] 05-03-PLAN.md — TripDetailHeader visibility toggle + Copy link button, /api/trips/[id]/visibility Edge route, login private-trip message

**Wave 2** *(blocked on Wave 1 completion)*
- [x] 05-04-PLAN.md — Vercel deployment + smoke test APPROVED 2026-05-19 — live at https://trailfilm.vercel.app
...
**Goal:** Migrate the cinematic viewer map from mapbox-gl to @vis.gl/react-google-maps, and replace the landing page WorldMapSection with an Aceternity-style animated arc world map using d3-geo projections and SVG arc animations.
**Requirements:** Phase 6 post-MVP feature phase — no formal REQ-IDs
**UI hint:** yes — viewer map migration (visual parity) + landing page world map enhancement
**Dependencies:** Phase 4 (viewer), Phase 5 (deployed Vercel)

**Success criteria:**
1. The cinematic viewer renders a dark Google Maps tile layer with amber dashed Polyline and amber AdvancedMarker dots at all chapter GPS coordinates — visual parity with Phase 4 Mapbox output.
2. Click-to-place pin mode works via Google Maps onClick; flyTo equivalent (panTo+setZoom) fires on chapter scroll.
3. The landing page WorldMapSection shows animated D3 arc paths drawing on scroll entrance; arcs connect the 8 JOURNEY_ARCS pairs across the world map; reduced motion renders static arcs.
4. mapbox-gl and @types/mapbox-gl are absent from package.json; NEXT_PUBLIC_MAPBOX_TOKEN is absent from .env.example.

**Plans:** 6 plans

Plans:
**Wave 1** *(parallel — disjoint files)*
- [ ] 06-01-PLAN.md — Package setup: install @vis.gl/react-google-maps + d3-geo, remove mapbox-gl, update .env.example, add markerPulse keyframe to globals.css
- [ ] 06-04-PLAN.md — WorldMap component: create src/components/ui/world-map.tsx from scratch (d3-geo geoNaturalEarth1, SVG arc animations, Framer Motion pathLength draw, animateMotion traveling dot, useInView scroll gate)

**Wave 2** *(blocked on Wave 1 completion — parallel within wave)*
- [ ] 06-02-PLAN.md — ViewerMap rewrite: full replacement of src/components/viewer/ViewerMap.tsx with @vis.gl/react-google-maps (Map, AdvancedMarker, Polyline, useMap, flyTo→panTo+setZoom, click-to-place)
- [ ] 06-03-PLAN.md — CinematicViewer APIProvider: wrap ViewerMap usage site with APIProvider in CinematicViewer.tsx
- [ ] 06-05-PLAN.md — Landing page replacement: delete WorldMapSection/JourneyPin/WORLD_PATHS/journeyPins from page.tsx; add JOURNEY_ARCS; replace with new WorldMapSection using WorldMap component

**Wave 3** *(blocked on Wave 2 completion)*
- [ ] 06-06-PLAN.md — Integration verification + human-action deployment steps (pnpm run build, D-13 API key restriction, D-14 Maps JS API enable, D-15 Vercel env vars)
...
| 4. Cinematic Viewer | 3/3 | Complete — Human Verified | 2026-05-19 |
| 5. Sharing & Deploy | 4/4 | Complete — Human Verified | 2026-05-19 |
| 6. Google Maps Migration & World Map | 0/6 | Planned (3 waves) | — |
