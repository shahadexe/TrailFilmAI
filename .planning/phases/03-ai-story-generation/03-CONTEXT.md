...
### Generation Approach
- **D-01:** Use **Gemini 2.5 Flash** (`gemini-2.5-flash`) — NOT 1.5 Flash as specified in the Build Guide, and NOT 2.0 Flash (retires June 1, 2026). 2.5 Flash is the direct replacement for 2.0 Flash with identical API surface and the same free tier rate limits (15 RPM / 1500 RPD). Decision updated 2026-05-19 based on research OQ-01.
...
### Regeneration Flow
- **D-13:** On `/trip/[id]` for a completed trip, a **"Change tone & regenerate"** option appears in the three-dot menu in `TripDetailHeader`. It opens a shadcn **Dialog** (NOT AlertDialog — `alert-dialog.tsx` is not installed in this project; `dialog.tsx` is the installed primitive, consistent with Phase 2 `DeleteTripDialog` pattern; UI-SPEC says "no new installs required"). The dialog contains the same 2×2 tone selector card grid (consistent with the wizard). Current tone is pre-selected. Updated 2026-05-19: Dialog substitution approved per plan-checker verification (alert-dialog.tsx absent from codebase).
...
- **D-15:** The regeneration UX during generation matches the wizard generation UX (amber indeterminate progress bar + rotating status text: "Reading your photos…" → "Weaving your story…" → "Almost ready…"), rendered inline on `/trip/[id]` below the TripDetailHeader. The dialog closes immediately on confirm; the page transitions to `generation_status = 'generating'` and shows the full `GenerationProgress` component (same component as wizard Step 3). Confirmed 2026-05-19 per user decision.
