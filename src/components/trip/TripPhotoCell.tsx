...
import type { PhotoForDisplay } from './TripPhotoGrid'
...
export function TripPhotoCell({ photo, index }: TripPhotoCellProps) {
  const shouldReduce = useReducedMotion()

  const parsedDate = photo.taken_at ? new Date(photo.taken_at) : null
  const hasValidTimestamp = parsedDate !== null && !isNaN(parsedDate.getTime())
  const hasBadge = hasValidTimestamp || photo.hasGps
          ...
          {hasValidTimestamp && (
            <div className="flex items-center gap-1">
              ...
              <span className="font-sans text-[12px] font-medium uppercase tracking-[0.05em] text-parchment-400">
                {formatExifTimestamp(parsedDate!)}
              </span>
            </div>
          )}
